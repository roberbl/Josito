...
import { z } from 'zod';
import { clearFirebaseSession, firebaseAuthRest, firestoreRest, setFirebaseSession, stringValue, timestampValue } from '@/lib/firebase';
...
export async function registerAction(_: unknown, formData: FormData) {
  ...
  if (!response.ok) return { error: response.data.error?.message ?? 'No se ha podido crear la cuenta.' };
